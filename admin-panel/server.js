...
const fs = require('fs');
const cleanup = require('../DB/cleanupConnections');
...
app.get('/api/stats', async (req, res) => {
    try {
        ...
        const connected = await ConfigModel.countDocuments({connectionStatus: true, trash: {$ne: true}});
...
app.get('/api/cleanup/stats', async (req, res) => {
    try {
        const stats = await cleanup.getConnectionStats();
        res.json(stats);
    } catch (e) {
        res.status(500).json({error: e.message});
    }
...
app.get('/api/cleanup/trash', async (req, res) => {
    try {
        const trash = await cleanup.ConfigModel.find({trash: true});
        res.json({trash});
    } catch (e) {
        res.status(500).json({error: e.message});
    }
...
app.post('/api/cleanup/move-old', async (req, res) => {
    try {
        const result = await cleanup.moveOldConnectionsToTrash();
        res.json(result);
    } catch (e) {
        res.status(500).json({error: e.message});
    }
...
// پاک‌سازی پیشرفته
app.post('/api/cleanup/advanced', async (req, res) => {
    try {
        const {daysOld, maxTests, dryRun, moveConnected} = req.body;
        const result = await cleanup.advancedCleanup({daysOld, maxTests, dryRun, moveConnected});
        res.json(result);
    } catch (e) {
        res.status(500).json({error: e.message});
    }
...
// بازگردانی کانفیگ(ها) از trash
app.post('/api/cleanup/restore', async (req, res) => {
    try {
        const {ids} = req.body;
        const result = await cleanup.restoreFromTrash(ids);
        res.json(result);
    } catch (e) {
        res.status(500).json({error: e.message});
    }
...
// حذف دائمی کانفیگ(ها) از trash
app.post('/api/cleanup/delete', async (req, res) => {
    try {
        const {ids} = req.body;
        const result = await cleanup.permanentlyDeleteFromTrash(ids);
        res.json(result);
    } catch (e) {
        res.status(500).json({error: e.message});
    }
...
// بازگردانی همه کانفیگ‌ها از trash
app.post('/api/cleanup/restore-all', async (req, res) => {
    try {
